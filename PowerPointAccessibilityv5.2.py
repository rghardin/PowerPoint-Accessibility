...
def extract_chart_data(chart):
    """Extract data and information from a chart shape for alt text generation"""
    chart_info = []
    
    try:
        # Get chart type
        chart_type = str(chart.chart_type).replace('_', ' ').title()
        chart_info.append(f"Chart Type: {chart_type}")
    except:
    ...
    try:
        ...
        for series in chart.series:
            ...
            if values:
                chart_info.append(f"Series '{series_name}': {', '.join(values[:10])}")  # Limit to first 10 values
            else:
                chart_info.append(f"Series: {series_name}")
    except:
        pass
    ...
    try:
        ...
        if chart.plots and len(chart.plots) > 0:
            ...
            if hasattr(plot, 'categories') and plot.categories:
                categories = [str(cat) for cat in plot.categories][:10]  # Limit to first 10
                if categories:
                    chart_info.append(f"Categories: {', '.join(categories)}")
    except:
        pass
    
    return "\n".join(chart_info) if chart_info else "Chart with unknown data"


def generate_chart_alt_text(api_key, model_id, chart_data, context=""):
    """Generate alt text/summary for a chart using the LLM"""
    prompt = f"""Generate a concise, descriptive alt text for a chart in a PowerPoint presentation.
    ...
    The alt text should:
    - Describe the type of chart and what it represents
    - Summarize the key data trends or comparisons shown
    - Be concise (under 200 characters if possible)
    - Be meaningful for screen reader users
...
def set_chart_alt_text(shape, alt_text, title=""):
    """Set alt text for a chart shape"""
    try:
        nvSpPr = shape._element._nvXxPr
        cNvPr = nvSpPr.cNvPr
        cNvPr.set('descr', alt_text)
        ...
        # Charts may have different element structure
        try:
            # Alternative approach for chart shapes
            nvGrpSpPr = shape._element.nvGraphicFramePr
            cNvPr = nvGrpSpPr.cNvPr
            cNvPr.set('descr', alt_text)
            if title:
                cNvPr.set('title', title)
        except:
...
def extract_group_shape_info(shape):
    """
    Extract information from a grouped shape for alt text generation.
    Recursively processes nested groups and extracts text content.
    
    Args:
        shape: A grouped shape object
    ...
    def process_shape(s):
        """Recursively process shapes in a group"""
        shape_info['shape_count'] += 1
        
        # Get shape type
        try:
            shape_type_name = str(s.shape_type).replace('MSO_SHAPE_TYPE.', '').replace('_', ' ').title()
            if shape_type_name not in shape_info['shape_types']:
                shape_info['shape_types'].append(shape_type_name)
        except:
            pass
        ...
        if s.has_text_frame:
            for paragraph in s.text_frame.paragraphs:
                text = paragraph.text.strip()
                if text:
                    shape_info['text_content'].append(text)
        
        # Process nested groups
        if s.shape_type == MSO_SHAPE_TYPE.GROUP:
            for child_shape in s.shapes:
...
def extract_auto_shape_info(shape):
    """
    Extract information from an auto shape for alt text generation.
    
    Args:
        shape: An auto shape object
    ...
    try:
        shape_info['shape_type'] = str(shape.shape_type).replace('MSO_SHAPE_TYPE.', '').replace('_', ' ').title()
    except:
        pass
    
    # Try to get the specific auto shape type (e.g., Rectangle, Arrow, Star)
    try:
        if hasattr(shape, 'auto_shape_type') and shape.auto_shape_type is not None:
            shape_info['auto_shape_type'] = str(shape.auto_shape_type).replace('MSO_AUTO_SHAPE_TYPE.', '').replace('_', ' ').title()
    except:
        pass
    ...
    try:
        if shape.line and shape.line.width is not None:
...
def generate_grouped_shape_alt_text(api_key, model_id, shape_info, context=""):
    """
    Generate alt text for a grouped shape (figure/diagram) using the LLM.
    
    Args:
        api_key: API key for LLM
        model_id: Model ID to use
        shape_info: Dictionary containing shape information
        context: Additional context from the slide
    
    Returns:
        Generated alt text string
    """
    # Build description of the grouped shape
    description_parts = []
    
    if shape_info['shape_count'] > 0:
        description_parts.append(f"A figure composed of {shape_info['shape_count']} shapes")
    
    if shape_info['shape_types']:
        types_str = ", ".join(shape_info['shape_types'][:5])  # Limit to 5 types
        description_parts.append(f"Shape types used: {types_str}")
    
    if shape_info['text_content']:
        text_str = " | ".join(shape_info['text_content'][:10])  # Limit to 10 text items
        description_parts.append(f"Text labels: {text_str}")
    
    shape_description = "\n".join(description_parts)
    
    prompt = f"""Generate concise, descriptive alt text for a figure/diagram in a PowerPoint presentation.
...
The alt text should:
- Describe what the figure represents (e.g., flowchart, diagram, process, organization chart)
- Mention key elements or labels if they help convey meaning
- Be concise (under 200 characters if possible)
- Be meaningful for screen reader users
- Not start with "Image of" or "Figure of"
...
def generate_auto_shape_alt_text(api_key, model_id, shape_info, context=""):
    """
    Generate alt text for an individual auto shape using the LLM.
    ...
    if shape_info['auto_shape_type']:
        description_parts.append(f"Shape type: {shape_info['auto_shape_type']}")
    elif shape_info['shape_type']:
        description_parts.append(f"Shape type: {shape_info['shape_type']}")
    
    if shape_info['text_content']:
        text_str = " ".join(shape_info['text_content'])
        description_parts.append(f"Text content: {text_str}")
    
    shape_description = "\n".join(description_parts)
    
    prompt = f"""Generate concise, descriptive alt text for a shape in a PowerPoint presentation.
...
The alt text should:
- Describe what the shape represents in the context of the presentation
- Include any text content if meaningful
- Be concise (under 125 characters if possible)
- Be meaningful for screen reader users
...
def set_shape_alt_text(shape, alt_text, title=""):
    """
    Set alt text for a shape (grouped or auto shape).
    
    Args:
        shape: The shape object
        alt_text: Alt text to set
        title: Optional title for the shape
    """
    try:
        # Try standard approach for most shapes
        nvSpPr = shape._element._nvXxPr
        cNvPr = nvSpPr.cNvPr
        cNvPr.set('descr', alt_text)
        if title:
        ...
        try:
            # Alternative for group shapes
            nvGrpSpPr = shape._element.nvGrpSpPr
            cNvPr = nvGrpSpPr.cNvPr
            cNvPr.set('descr', alt_text)
            if title:
                cNvPr.set('title', title)
...
def should_process_shape(shape):
    """
    Determine if a shape should have alt text generated.
    Filters out simple shapes that don't need descriptions.
    
    Args:
        shape: The shape to evaluate
    
    Returns:
        Boolean indicating if the shape should be processed
    """
    # Skip very small shapes (likely decorative)
    try:
        if shape.width < 100000 and shape.height < 100000:  # Less than ~1 inch
            return False
    except:
        pass
    
    # Process grouped shapes (likely diagrams/figures)
    if shape.shape_type == MSO_SHAPE_TYPE.GROUP:
        return True
    
    # Process auto shapes that contain text or are significant
    if shape.shape_type == MSO_SHAPE_TYPE.AUTO_SHAPE:
        # If it has text, it's probably meaningful
        if shape.has_text_frame:
            for paragraph in shape.text_frame.paragraphs:
                if paragraph.text.strip():
                    return True
        # Large shapes are likely important
        try:
            if shape.width > 500000 or shape.height > 500000:  # More than ~0.5 inches
                return True
        except:
            pass
        ...
        for shape in slide.shapes:
            ...
            #Process embedded charts:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            existing_alt = shape._element.nvGraphicFramePr.cNvPr.get('descr', '')
                        except:
                            pass
                    ...
                    if not existing_alt or existing_alt.strip() == "":
                        ...
                        alt_text = generate_chart_alt_text(
                            api_key, model_id, chart_data, slide_context
                        )
                        
                        st.info(f"**Slide {slide_idx + 1} - Chart:**\n{alt_text}")
                        alt_text_log.append({
                            'slide': slide_idx + 1,
                            'type': 'Chart',
                        ...
                        set_chart_alt_text(shape, alt_text)
                        charts_processed += 1
                        log_messages.append(f"  Slide {slide_idx + 1}: Added alt text to chart")
                except Exception as e:
                    log_messages.append(f"  Slide {slide_idx + 1}: Error processing chart - {str(e)}")
            ...
            #Process grouped shapes:
                ...
                try:
                    ...
                    existing_alt = ""
                    try:
                        existing_alt = shape._element.nvGrpSpPr.cNvPr.get('descr', '')
                    except:
                        pass
                    
                    if not existing_alt or existing_alt.strip() == "":
                        # Extract shape information
                        shape_info = extract_group_shape_info(shape)
                        
                        # Only process if it seems like a meaningful figure
                        if shape_info['shape_count'] > 1 or shape_info['text_content']:
                            # Generate alt text using LLM
                            alt_text = generate_grouped_shape_alt_text(
                                api_key, model_id, shape_info, slide_context
                            )
                            
                            st.info(f"**Slide {slide_idx + 1} - Grouped Shape ({shape_info['shape_count']} shapes):**\n{alt_text}")
                            alt_text_log.append({
                                'slide': slide_idx + 1,
                                'type': 'Grouped Shape',
                            ...
                            set_shape_alt_text(shape, alt_text)
                            shapes_processed += 1
                            log_messages.append(f"  Slide {slide_idx + 1}: Added alt text to grouped shape")
                except Exception as e:
                    log_messages.append(f"  Slide {slide_idx + 1}: Error processing grouped shape - {str(e)}")
            
            #Process auto shapes:
            elif shape.shape_type == MSO_SHAPE_TYPE.AUTO_SHAPE:
                try:
                    # Only process significant auto shapes
                    if should_process_shape(shape):
                        # Check for existing alt text
                        existing_alt = ""
                        try:
                            existing_alt = shape._element._nvXxPr.cNvPr.get('descr', '')
                        ...
                        if not existing_alt or existing_alt.strip() == "":
                            # Extract shape information
                            shape_info = extract_auto_shape_info(shape)
                            
                            # Generate alt text using LLM
                            alt_text = generate_auto_shape_alt_text(
                                api_key, model_id, shape_info, slide_context
                            )
                            
                            st.info(f"**Slide {slide_idx + 1} - Shape ({shape_info.get('auto_shape_type', 'Auto Shape')}):**\n{alt_text}")
                            alt_text_log.append({
                                'slide': slide_idx + 1,
                                'type': 'Auto Shape',
                                'method': 'Shape analysis',
                            ...
                            set_shape_alt_text(shape, alt_text)
                            shapes_processed += 1
                            log_messages.append(f"  Slide {slide_idx + 1}: Added alt text to shape")
                except Exception as e:
                    log_messages.append(f"  Slide {slide_idx + 1}: Error processing auto shape - {str(e)}")
    
    log_messages.append(
        f"✓ Processed {images_processed} images "
        f"({vision_used_count} with vision, {text_fallback_count} text-only), "
        f"{tables_processed} tables, {charts_processed} charts, and {shapes_processed} shape figures"
    )
