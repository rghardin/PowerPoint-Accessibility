...
def convert_image_to_supported_format(image_blob):
    ...
    Uses Wand (ImageMagick) for EMF/WMF conversion on Linux.
    ...
    # Try Wand (ImageMagick) for EMF/WMF conversion
    if original_format.lower() in ['emf', 'wmf']:
        try:
            ...
            st.warning("Wand library not available for EMF conversion")
        except Exception as e:
            st.warning(f"Wand conversion failed: {str(e)}")
...
def create_handout_from_slides_pdf(slides_pdf_bytes, slides_per_page=3):
    ...
    layouts = {
        ...
        3: [
            (0.52, 0.03, 0.98, 0.30),   # Top right
            (0.52, 0.35, 0.98, 0.62),   # Middle right
            (0.52, 0.68, 0.98, 0.95)    # Bottom right
            # Left side is for notes (blank)
        ],
        4: [
    ...
    for group_start in range(0, len(doc), num_positions):
        ...
        for i, pos in enumerate(positions):
            ...
            new_page.draw_rect(rect, color=(0.7, 0.7, 0.7), width=0.5)
