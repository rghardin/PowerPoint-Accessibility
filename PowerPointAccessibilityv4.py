...
def convert_pptx_to_pdf_batch_libreoffice(pptx_files_dict, temp_dir, generate_slides=False, generate_handouts=False, slides_per_page=3):
    ...
    for filename, pptx_bytes in pptx_files_dict.items():
        ...
        if generate_slides:
            slides_pdf_path = convert_pptx_to_slides_pdf_libreoffice(temp_pptx_path, temp_dir)
            if slides_pdf_path and os.path.exists(slides_pdf_path):
                # Rename to indicate it's slides
                final_slides_path = os.path.join(temp_dir, f"{base_name}_slides.pdf")
                if slides_pdf_path != final_slides_path:
                    os.rename(slides_pdf_path, final_slides_path)
                with open(final_slides_path, 'rb') as f:
                    slides_pdf_dict[f"{base_name}_slides.pdf"] = BytesIO(f.read())
                st.write(f"  ✓ Slides PDF created for {filename}")
            else:
                st.warning(f"  ✗ Failed to create slides PDF for {filename}")
        
        # Generate handouts PDF if requested
        if generate_handouts:
            handouts_pdf_path = convert_pptx_to_handout_pdf_libreoffice(
                temp_pptx_path, temp_dir, slides_per_page
            )
            if handouts_pdf_path and os.path.exists(handouts_pdf_path):
                ...
                st.warning(f"  ✗ Failed to create handouts PDF for {filename}")
